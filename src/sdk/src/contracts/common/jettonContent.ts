...
export async function unpackJettonOnchainMetadata (content: Cell, jetton : boolean = true, additional?: string[])  : Promise<{[key: string]: string}>
{
    const cs = content.beginParse();
    const contentType = cs.loadUint(8);
    if(contentType == 1) {
...
export async function packJettonOnchainMetadata(data: { [s: string]: string | undefined }): Promise<Cell> {
